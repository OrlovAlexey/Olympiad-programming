...
inline bool is_between_x2(Point p, Point a1, Point a2) {
    return (((p.x < a1.x || cmp(p.x, a1.x)) && (p.x > a2.x)) || ((p.x > a1.x || cmp(p.x, a1.x)) && (p.x < a2.x)));
}

inline bool is_between_y2(Point p, Point a1, Point a2) {
    return (((p.y < a1.y || cmp(p.y, a1.y)) && (p.y > a2.y)) || ((p.y > a1.y || cmp(p.y, a1.y)) && (p.y < a2.y)));
}

inline bool is_between2(Point p, Point a1, Point a2) {
    if (p == a1) {
        return true;
    }
    if (p == a2) {
...
//
//        }
        if (cmp(p1.x, ray.p.x)) throw 1;
        bool is_b = is_between2(p1, a1, a2);
        return is_b;
    } else {
        // the segment is parallel to ray

//        return (l.is_belong(a1) && (a1.x >= p.x || a2.x >= p.x));
        return false;
    }
}
